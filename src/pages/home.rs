...
pub async fn login_handler(
    ...
    let pool = PgPoolOptions::new()
        ...
        .connect("postgres://postgres:example@localhost:5432/b2rust").await;

    // melakukan query
    let row  = sqlx::query_as::<_, User>("SELECT id, email, name, role from users where email = $1 and password = $2")
        .bind(form.email.clone())
        .bind(form.password.clone())
        .fetch_one(&pool.unwrap()).await;
